...
SHORT_TERM_NUM_VALUES = SHORT_TERM_DAYS * 24 * 60 // SHORT_TERM_VALUES_PER_AGGREGATE
MEDIUM_TERM_VALUES_PER_AGGREGATE = 5
MEDIUM_TERM_NUM_VALUES = MEDIUM_TERM_DAYS * 24 * 60 // MEDIUM_TERM_VALUES_PER_AGGREGATE
LONG_TERM_VALUES_PER_AGGREGATE = 10
LONG_TERM_NUM_VALUES = LONG_TERM_DAYS * 24 * 60 // LONG_TERM_VALUES_PER_AGGREGATE
    ...
    def setup_connection(self):
        ...
        while True:
            ...
            if pid == 0:
                ...
                logger.debug('TCP: connected to inverter: %s:%s' % (str(addr), str(conn)))
    ...
    def run(self):
        ...
        while True:
            ...
            while True:
                ...
                if len(data) == 0:
                    logger.warn( "received 0 bytes - closing connection and restarting")
                    conn.close()
                    tcp.close()
                    break
    ...
    def createDatabase( self ):
        ...
        if not os.path.exists( self.rrd_power ):
            ...
            for profile in DAILY_PROFILES:
                ...
                mi, ma = RANGES[prefix]
                data_sources.append( 'DS:%s:GAUGE:120:%i:%i' % (profile, mi, ma) )
        ...
        if not os.path.exists( self.rrd_energy ):
            ...
            for profile in DAILY_ENERGY:
                prefix, suffix = profile.split("_")
                mi, ma = RANGES[prefix]
                data_sources.append( 'DS:%s:GAUGE:1400:%i:%i' % (profile, mi, ma) )
